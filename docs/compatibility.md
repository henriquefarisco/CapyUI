...
`latest` or `v<semver>`, checked out exactly, matched against `VERSION` and the
Git tag, and passed unchanged to `PUBLISH_TAG`. GitHub Actions pins were also
refreshed while remaining locked to immutable commit SHAs.
