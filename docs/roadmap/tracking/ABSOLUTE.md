...
- **Em produção (commit-side):** `2.22.0` (tags externas pendentes). **Fase 1.x linear CONCLUÍDA + fase 2.x major CONCLUÍDA (14/14)** + **trilha 2.14–2.21 de estado dos advanced widgets COMPLETA (8/8)** + **multi-touch gestures (v2.22, completa a 1.4)**.
...
- **Trilha aditiva:** estado dos advanced widgets **COMPLETA 8/8** (2.14 date + 2.15 color + 2.16 table + 2.17 autocomplete + 2.18 tree + 2.19 chart + 2.20 rich-text + 2.21 canvas draw); **v2.22 retoma o backlog deferido** com multi-touch (pinch/rotate, completa a 1.4). Próximo trabalho aditivo: resto do backlog deferido e/ou deprecações pré-3.0 (ADR-0006).
...
- Testes: **345** funções em `tests/test_widget_contracts.c` (… + 6 v2.13 + 8 v2.14 + 8 v2.15 + 8 v2.16 + 8 v2.17 + 8 v2.18 + 8 v2.19 + 8 v2.20 + 8 v2.21 + 8 v2.22). Contagem pinada em `make version-check` (defs == call-sites).
...
- LOC tests: ~7.6k.
- ABI `capy-ui-widget` ativas: 0.0–0.15 (com 0.7 e 0.12 reservadas), 1.0–1.10 (toda fase 1.x linear, em LTS ≥12m), 2.0–2.21, **2.22 (atual, aditivo)**.
