...
- **Em produção (tagged, validated):** `2.13.1` (commit-side; tags externas pendentes). **Fase 1.x linear CONCLUÍDA + fase 2.x major CONCLUÍDA (14/14)**.
...
- **Próximo marco:** **fase 3.x (Vision platform)** — quebra controlada via novo major bump 3.0 (DL schema 9, sketch surface, GPU integration, cross-device handoff). Sem fatias 2.x.minor pendentes.
...
- Testes: **272** funções em `tests/test_widget_contracts.c` (97 1.0 + 7 v1.1 + 7 v1.2 + 7 v1.3 + 7 v1.4 + 7 v1.5 + 8 v1.6 + 7 v1.7 + 7 v1.8 + 7 v1.9 + 7 v1.10 + 8 v2.0 + 8 v2.1 + 7 v2.2 + 7 v2.3 + 8 v2.4 + 7 v2.5 + 7 v2.6 + 7 v2.7 + 8 v2.8 + 8 v2.9 + 8 v2.10 + 8 v2.11 + 7 v2.12 + 6 v2.13).
- LOC core (`src/widget/*.c` + `*.h`): ~2.3k.
- LOC desktop-session (`src/desktop/` + `src/window/` + `src/apps/`): ~8.8k.
- LOC tests: ~1280.
- ABI `capy-ui-widget` ativas: 0.0–0.15 (com 0.7 e 0.12 reservadas), 1.0–1.10 (toda fase 1.x linear, em LTS ≥12m), 2.0–2.12, **2.13 (atual, aditivo, última do 2.x)**.
