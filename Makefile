# CapyUI Makefile — 2.22.0 — ABI capy-ui-widget 2.22 (multi-touch gestures: pinch + rotate)
#
...
version-check:
	test "$(VERSION)" = "2.22.0"
	grep -q "Version: 2.22.0" README.md
	test "$$(grep -cE '^  test_[a-z0-9_]+\(\);' tests/test_widget_contracts.c)" = "345"
	test "$$(grep -cE '^static void test_[a-z0-9_]+\(void\)' tests/test_widget_contracts.c)" = "345"
