# CapyUI Makefile — 2.22.1 — ABI capy-ui-widget 2.22 (multi-touch gestures: pinch + rotate)
#
...
version-check:
	test "$(VERSION)" = "2.22.1"
	grep -q "Version: 2.22.1" README.md
	test "$$(grep -cE '^  test_[a-z0-9_]+\(\);' tests/test_widget_contracts.c)" = "347"
	test "$$(grep -cE '^static void test_[a-z0-9_]+\(void\)' tests/test_widget_contracts.c)" = "347"
