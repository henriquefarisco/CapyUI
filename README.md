...
```
https://github.com/<owner>/CapyUI/releases/download/v2.24.0/modules-index.txt
```
...
CapyOS first-boot releases should pin the index URL to a semver release tag, for example `https://github.com/<owner>/CapyUI/releases/download/v2.24.0/modules-index.txt`. This avoids an extra GitHub `latest` redirect during kernel-space module installation.

To freeze a semver release, push a `v*` tag (for example `v2.24.0`). The same workflow publishes that immutable release and marks it as Latest.
